var path = require('path')
	...
	, request = require('request').defaults({headers:{'User-Agent': 'buddy'}})
	, http = require('http')
	, unzip = require('unzip')
	...
	, term = {
		debug: function(){},
		strong: function(){}}
	, ua = 'Buddy (http://github.com/popeindustries/buddy) Node.js/' + process.version + ' ' + process.platform;
...
Dependency.prototype.validateVersion = function(fn) {
	...
	if (!RE_VALID_VERSION.test(this.version)) {
		...
		3);
		// Get tags
		request({url:"https://api.github.com/repos/" + this.name + "/tags",
			json: true,
	...
	if (!this.keep) {
		...
		async.forEachSeries(this.resources, (function(resource, cb) {
			...
			recurfs[self.local ? 'cp' : 'mv'](resource, self.destination, true, function(err, filepath) {
				...
				idx++;
				if (!~filepath.indexOf(self.temp)) self.files.push(path.relative(process.cwd(), filepath));
